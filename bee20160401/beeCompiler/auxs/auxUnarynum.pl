...
unarynumNewInDomain(Dom,_):-
   throw(error(unknown_domain(Dom))).
...
unarynumDivByConst(A,-1,NA):-!,
    unarynumNeg(A,NA).
unarynumDivByConst(A,Const,(NMin,NMax,DBits,DLBit)):-!,
    (Const > 0 ->
        A=(Min,Max,Bits,_),
        NMin is Min // Const,
        NMax is Max // Const,
    ...
    ;
        unarynumNeg(A,(Min,Max,Bits,_)),
        ConstAbs is -Const,
        NMin is Min // ConstAbs,
        NMax is Max // ConstAbs,
        (NMin < NMax ->
            Drop is ConstAbs - Min mod ConstAbs - 1,
            Const1 is ConstAbs - 1,
            auxLists:listDropFrom(Drop,Bits,Bits1),!,
            keepIthBits(Bits1,Const1,DBits,DLBit)
        ;
            DBits=[],
