...
:- Head=bool_array_element(Indx,Vec,Z,ConstrsH-ConstrsT),
   Body=(
       ...
       (Min==0 ->
           bcBoolElement:boolElementSimplify(bc(Type,[IBits,Vec,Z]),Constr,_)
       ;
       (Min < 0 ->
           Xdrop is abs(Min),
           (auxLists:listSplit(Xdrop,IBits,NoIndx,RIBits) ->
               auxLiterals:litAsgnFalses(NoIndx),
               bcBoolElement:boolElementSimplify(bc(Type,[RIBits,Vec,Z]),Constr,_)
           ;
               throw(unsat)
           )
       ;
       ...
       (auxLists:listDropFrom(Min,Vec,RVec)->
           bcBoolElement:boolElementSimplify(bc(Type,[IBits,RVec,Z]),Constr,_)
       ;
           throw(unsat)
       ))),
       (Constr==none ->
