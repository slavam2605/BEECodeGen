...
sumBitsLEqKSimplify_step2(bc(Type,[SumUB|WBits]),Constr):-!,
     ...
     (SumLB == 1 ->
         ...
         bcAtLeastOne:aloSimplify(bc(ALOtype,Vec),Constr,_) ;
     (ContainGt1 \==1 ; sumBitsLBsimplifyF(NWBits,MaxWSum,SumLB,Changed)),!,
     (Changed==1 ->
         sumBitsLEqKSimplify_step1(bc(Type,[SumUB|NWBits]),Constr,_)
     ;
