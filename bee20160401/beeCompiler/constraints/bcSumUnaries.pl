...
sumUnariesSimplifyAdv(bc(Type,[EqTo|WInts]),Constr,Changed):-
    extractMax1(WInts,RWInts,WBits),
    (RWInts=[] ->
         Changed=1,
         bcSumBits:sumBitsType(SumType),
         bcSumBits:sumBitsSimplify_step1(bc(SumType,[(EqTo,EqTo,[],1)|WBits]),Constr,1) ;
    (RWInts=[X] ->
         Changed=1,
         X=(Weight1,Int1u),
         auxUnarynum:unarynumMulByConst((0,Int1u),Weight1,NSInt1),
    ...
    (Changed==1 ->
       ...
       %%% TODO GCD(NWInts)
       (checkSpecialCases(NWInts, FEqTo, Constr) ;
        Constr=bc(Type,[FEqTo|NWInts]))
    ;
       %%% TODO GCD(WInts)
       Constr=bc(Type,[EqTo|WInts])
    )))).
