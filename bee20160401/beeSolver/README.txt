...
Usage:
======
After compile (see below) the executable have two modes: Solver and Checker.
...
%  load pl-satSolver ... % SWI-Prolog interface to CryptoMiniSAT v2.5.1 ... OK
...
since BumbleBEE using PL-CryptoMinisat.
...
The makefile will compile BumbleBEE and place two files in the "root BEE" directory:
BumbleBEE - Linux executable
and
pl-crypminisat.so - PL-CryptoMinisat library which will be loaded by
...
The batch file will place two files in the "root BEE" directory:
BumbleBEE.exe - Windows executable
and
pl-crypminisat.pl - PL-CryptoMinisat library which will be loaded by
                    BumbleBEE when execute (copied from ./satsolver folder).

In addition you required to copy the following files from SWI-Prolog directory:
swipl.dll
pthreadVC.dll
readutil.dll
